...
//  hw4
//
//  Created by Sadhana Vadrevu on 3/5/19.
...
int main()
{
    ...
    // destruction time.
    {
        ...
        vector<Store*> auxStoresPtr;
        for (int i = 0; i < auxStores.size(); i++) {
            auxStoresPtr.push_back(&auxStores[i]);
        }
        
        sort(auxStoresPtr.begin(), auxStoresPtr.end(), compareStorePtr);
        
        for (int i = 0; i < auxStoresPtr.size(); i++) {
            stores[i] = *auxStoresPtr[i];
        }
